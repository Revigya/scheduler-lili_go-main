...
int main() {
    ...
    while (1) {
        ...
        case PROC_SETPRIO:
            ...
            */
            {
                int target_pid = atoi(req->argv[1]);
                int new_priority = atoi(req->argv[2]);
                
                /* Validate priority range */
                if (new_priority >= 1 && new_priority <= 10) {
                    /* Set the priority using the grass interface */
                    grass->proc_set_priority(target_pid, new_priority);
                    reply->type = CMD_OK;
                } else {
...
static int app_spawn(struct proc_request *req) {
    ...
    grass->proc_set_priority(app_pid, 2);
